 ...
 *  @author Kevin Wayne
 */
public final class ByteArrayIn {
    private static final int EOF = -1;      // end of file

    private static ByteArrayInputStream in;  // input stream
    private static int buffer;              // one character buffer
    private static int n;                   // number of bits left in buffer
    private static boolean isInitialized;   // has BinaryStdIn been called for first time?

    // don't instantiate
    public ByteArrayIn(byte [] source) {
        initialize(source);
    }

    // fill buffer
    public static void initialize(byte [] source) {
   ...
   /**
    ...
    public static void main(String[] args) {
        ...
        while (!ByteArrayIn.isEmpty()) {
            char c = ByteArrayIn.readChar();
